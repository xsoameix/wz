...
int
wz_utf16le_size(uint8_t * size, const uint8_t * bytes) {
...
int
wz_utf16le_to_unicode(uint32_t * code, const uint8_t * bytes) {
  if ((bytes[1] & 0xfc) == 0xd8)
    if ((bytes[3] & 0xfc) == 0xdc)
...
int
wz_unicode_to_utf8_size(uint8_t * size, uint32_t code) {
  if ((code & 0xffffff80) == 0)      return * size = 1, 0;
  else if ((code & 0xfffff800) == 0) return * size = 2, 0;
  else if ((code & 0xffff0000) == 0) return * size = 3, 0;
  else if (code < 0x00110000)        return * size = 4, 0;
  else return 1;
}

int
wz_unicode_to_utf8(uint8_t * bytes, uint32_t code) {
